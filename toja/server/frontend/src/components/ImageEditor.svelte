<script lang="ts">
    ...
    function saveEdit() {
        saveJsonApiObject(selectedElement.joke);
        mode = MODE_SELECT;
        editMenuElement.classList.add('hidden');
        selectedElement = null;
        for (let element of elements) {
    ...
    function deleteElement() {
        deleteJsonApiObject('jokes', selectedElement.joke.id);
        elements = elements.filter((element) => { return element === selectedElement});
        app.stage.removeChild(selectedElement.graphics);
        mode = MODE_SELECT;
        editMenuElement.classList.add('hidden');
    ...
    <nav class="flex-none">
        <ul class="flex flex-row space-x-2">
            <li role="presentation">
                <button on:click={addNew} class="block p-2 {mode === MODE_EDIT ? 'text-gray-400' : 'text-accent'}" disabled={mode === MODE_EDIT}>
        ...
        <nav bind:this={editMenuElement} class="hidden absolute z-10 w-min bg-white/90 border border-primary shadow-lg">
            <ul class="flex flex-row space-x-2">
                {#if selectedElement && selectedElement.joke.id}
                    <li role="presentation">
                        <button on:click={deleteElement} class="block p-2 text-accent" aria-label="Delete the joke">
                ...
                {/if}
                <li role="presentation">
                    <button on:click={cancelEdit} class="block p-2 text-accent" aria-label="Discard any changes">
                        <svg viewBox="0 0 24 24" class="w-6 h-6" aria-hidden="true">
                ...
                <li role="presentation">
                    <button on:click={saveEdit} class="block p-2 text-accent" aria-label="Save changes">
                        <svg viewBox="0 0 24 24" class="w-6 h-6" aria-hidden="true">
