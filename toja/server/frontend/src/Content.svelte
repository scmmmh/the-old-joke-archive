...
<article class="container mx-auto mb-12">
    ...
    {#if $isGroupAdmin || $isGroupAdminUsers}
        <Route path="/admin/*"
            >{#if Admin}<svelte:component this={Admin} />{:else}<Loading
                />{/if}</Route
        >
    {/if}
    <Route path="/contribute"><Contribute /></Route>
    {#if $isGroupEditor}
        <Route path="/contribute/workbench"
            >{#if Workbench}<svelte:component this={Workbench} />{:else}<Loading
                />{/if}</Route
        >
    {/if}
    {#if $isGroupDataProvider}
        <Route path="/contribute/data"
            >{#if DataProvision}<svelte:component
                    this={DataProvision}
                />{:else}<Loading />{/if}</Route
        >
    {/if}
    <Dialog />
</article>
